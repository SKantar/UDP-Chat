...
public class PrivateRoomView extends JPanel{
	...
	public PrivateRoomView(User user) {
		this.user = new User(user.toString(), user.getID());
		this.ID = user.getID();
	...
	public void send(String message, boolean text) {
		...
		if (text) {
			...
			console(message);
			message = "/p/"+user.getID()+"/"+AppCore.getInstance().getClient().getID()+"/" + message + "/e/";
