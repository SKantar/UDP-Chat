...
public class ClosableTabbedPane extends JTabbedPane{
	...
	private class TabCloseUI implements MouseListener, MouseMotionListener {
		...
		public void mouseReleased(MouseEvent me) {
			if(closeUnderMouse(me.getX(), me.getY())){
				...
				if (isToCloseTab && selectedTab > -1){	
					...
					System.out.println(tabbedPane.getTitleAt(selectedTab));
					int cout = AppCore.getInstance().getPrivateRooms().size();
					for(int i = 0 ; i < cout; i++){
						if(tabbedPane.getTitleAt(selectedTab).contains(AppCore.getInstance().getPrivateRooms().get(i).getUser().getID()+"")){
							AppCore.getInstance().getPrivateRooms().remove(i);
							break;
						}
					}
				...
				//clearing the close button memory
				if(selectedTab > -1){
					closeX = tabbedPane.getBoundsAt(selectedTab).x + tabbedPane.getBoundsAt(selectedTab).width -width-5;
					closeY = tabbedPane.getBoundsAt(selectedTab).y +5;										
				} else {
					closeX = 0;
					closeY = 0;
				}
		...
		private void drawClose(Graphics g, int x, int y) {
			if(tabbedPane != null && tabbedPane.getTabCount() > 1){
				...
				drawColored(g2, isUnderMouse(x,y)? Color.PINK : Color.WHITE, x, y);
			}
		}
	...
	public boolean existsTabWithID(int ID){
		int tabCount = super.getTabCount();
		for(int i = 0 ; i < tabCount; i++){
			//System.out.println(getTitleAt(i));
			if(getTitleAt(i).contains(ID+""))
				return true;
		}
		return false;
	}
